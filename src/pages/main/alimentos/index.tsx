...
const FOODS_PAGE_SIZE = 4;

function getGrupoAlimentarLabel(grupoAlimentar: FoodItem['grupoAlimentar']) {
  if (!grupoAlimentar) {
    return '-';
  }
  ...
  return grupoAlimentar.nome ?? String(grupoAlimentar.id ?? '-');
}

function getFoodImagePreview(image64: string) {
  if (!image64) {
    return '';
  }
...
export default function FoodsPage() {
  ...
  const [page, setPage] = useState<PageResponse<FoodItem> | null>(null);
  const [loading, setLoading] = useState(true);
  const [currentPage, setCurrentPage] = useState(0);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedGroup, setSelectedGroup] = useState<'TODOS' | 'VERDE' | 'AMARELO' | 'AZUL' | 'VERMELHO'>('TODOS');
  const [isCreateFoodOpen, setIsCreateFoodOpen] = useState(false);
  const [isSavingFood, setIsSavingFood] = useState(false);
  const [isDeleteFoodOpen, setIsDeleteFoodOpen] = useState(false);
  ...
  const loadFoods = useCallback(async (pageIndex = 0, searchValue = '') => {
    setLoading(true);

    try {
      const response = await getFoods(pageIndex, FOODS_PAGE_SIZE, searchValue, selectedGroup === 'TODOS' ? '' : selectedGroup);
  ...
  useEffect(() => {
    const handleSearch = window.setTimeout(() => {
      void loadFoods(currentPage, searchTerm.trim());
    }, 300);

    return () => window.clearTimeout(handleSearch);
  }, [currentPage, searchTerm, loadFoods]);

  function handleOpenCreateFood() {
    setFormError('');
    setIsCreateFoodOpen(true);
  }

  function handleSelectGroup(group: 'TODOS' | 'VERDE' | 'AMARELO' | 'AZUL' | 'VERMELHO') {
    setCurrentPage(0);
    setSelectedGroup(group);
  }

  function handleCloseCreateFood() {
  ...
  async function handleCreateFood(event: FormEvent<HTMLFormElement>) {
    ...
    if (!formData.imagem) {
      setFormError('Selecione a imagem do alimento.');
    ...
    try {
      ...
      setFormError('Não foi possível cadastrar o alimento. Verifique os dados e tente novamente.');
    } finally {
      setIsSavingFood(false);
    }
  }

  function handleOpenDeleteFood(food: FoodItem) {
    setDeleteFoodTarget(food);
    setDeletePassword('');
    setDeleteError('');
    setShowDeletePassword(false);
    setIsDeleteFoodOpen(true);
  }

  function handleOpenFoodDetails(food: FoodItem) {
    setSelectedFood(food);
  }

  function handleCloseFoodDetails() {
    setSelectedFood(null);
  }

  function handleCloseDeleteFood() {
    setIsDeleteFoodOpen(false);
  ...
  async function handleDeleteFood(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    ...
    try {
      await deleteFood(deleteFoodTarget.id, deletePassword);
      ...
      setDeleteError('Não foi possível excluir o alimento. Verifique a senha e tente novamente.');
    } finally {
    ...
    <div className="flex min-h-screen w-full bg-[#F5F5F5]">
      <aside className="flex min-h-screen w-3/20 flex-col bg-white p-6 shadow-md">
        ...
        <div className="border-t border-gray-100 pt-6">
          <button type="button" onClick={() => void handleLogout()} className="w-full rounded-xl bg-red-500 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-red-300 focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-2">
            Logout
          </button>
        </div>
      </aside>
      ...
      <div className="flex min-h-screen w-17/20 flex-col items-center pb-0">
        <header className="w-[96%] overflow-hidden rounded-b-3xl bg-white shadow-sm">
          ...
          <div className="border-t border-slate-200 px-6 py-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <h1 className="text-2xl font-bold text-slate-900">Alimentos</h1>
              </div>
              ...
              <div className="flex w-full flex-col gap-3 md:w-auto md:flex-row md:items-center">
                ...
                <button type="button" onClick={handleOpenCreateFood} className="flex h-10 items-center gap-2 rounded-full border border-blue-500 bg-white px-4 text-sm font-semibold text-blue-700 shadow-sm transition-colors hover:bg-blue-50 whitespace-nowrap">
                  <AddUserIcon />
                  Cadastrar alimento
                </button>
              </div>
        ...
        <section className="mt-4 flex w-[96%] flex-1 flex-col overflow-hidden rounded-t-4xl rounded-b-none border border-gray-100 bg-white shadow-sm">
          <div className="flex items-center justify-between px-6 py-7">
            <h1 className="text-2xl font-semibold text-slate-900">Lista de alimentos cadastrados</h1>

            <div className="flex flex-wrap items-center gap-3">
              <button
                type="button"
                onClick={() => handleSelectGroup('TODOS')}
                className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${selectedGroup === 'TODOS' ? 'border border-slate-900 bg-slate-900 text-white' : 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}
              >
                Todos
              </button>
              <button
                type="button"
                onClick={() => handleSelectGroup('VERDE')}
                className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${selectedGroup === 'VERDE' ? 'border border-emerald-700 bg-emerald-600 text-white' : 'border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100'}`}
              >
                Verde
              </button>
              <button
                type="button"
                onClick={() => handleSelectGroup('AMARELO')}
                className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${selectedGroup === 'AMARELO' ? 'border border-yellow-600 bg-yellow-500 text-white' : 'border border-yellow-200 bg-yellow-50 text-yellow-700 hover:bg-yellow-100'}`}
              >
                Amarelo
              </button>
              <button
                type="button"
                onClick={() => handleSelectGroup('AZUL')}
                className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${selectedGroup === 'AZUL' ? 'border border-blue-700 bg-blue-600 text-white' : 'border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100'}`}
              >
                Azul
              </button>
              <button
                type="button"
                onClick={() => handleSelectGroup('VERMELHO')}
                className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${selectedGroup === 'VERMELHO' ? 'border border-red-700 bg-red-600 text-white' : 'border border-red-200 bg-red-50 text-red-700 hover:bg-red-100'}`}
              >
                Vermelho
              </button>
            </div>
      ...
      {selectedFood && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4" onClick={handleCloseFoodDetails}>
          <div className="max-h-[92vh] w-[90vw] max-w-[90vw] overflow-hidden rounded-3xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-5">
              <div>
                <h2 className="text-2xl font-semibold text-slate-900">Detalhes do alimento</h2>
              </div>

              <button type="button" onClick={handleCloseFoodDetails} className="rounded-full p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900" aria-label="Fechar detalhes do alimento">
            ...
            <div className="grid max-h-[calc(92vh-92px)] gap-6 overflow-y-auto p-6 lg:grid-cols-[320px_minmax(0,1fr)]">
              <div className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 shadow-sm">
                {getFoodImagePreview(selectedFood.imagem64) ? (
                  <img src={getFoodImagePreview(selectedFood.imagem64)} alt={`Imagem do alimento ${selectedFood.nomePrincipal}`} className="aspect-square h-full min-h-80 w-full object-cover" />
                ) : (
              ...
              <div className="space-y-5">
                <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">{selectedFood.grupoAlimentar ? getGrupoAlimentarLabel(selectedFood.grupoAlimentar) : '-'}</span>
                  </div>
                  <h3 className="mt-4 text-3xl font-bold tracking-tight text-slate-900">{selectedFood.nomePrincipal}</h3>
                </div>
                ...
                <DetailSection title="Texto informativo" value={selectedFood.textoInformativo} />
              </div>
            </div>
          </div>
        </div>
      ...
      {isDeleteFoodOpen && deleteFoodTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 px-4" onClick={handleCloseDeleteFood}>
          <div className="w-full max-w-130 rounded-[28px] bg-white px-10 py-10 shadow-[0_30px_80px_rgba(15,23,42,0.22)]" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-4">
              <h2 className="text-[1.55rem] font-medium text-slate-900">Deletar alimento</h2>
              <button type="button" onClick={handleCloseDeleteFood} className="rounded-full p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900" aria-label="Fechar popup">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><path d="M18 6 6 18" /><path d="m6 6 12 12" /></svg>
              </button>
            </div>
            ...
            <form className="mt-8" onSubmit={handleDeleteFood}>
              ...
              <div className="mt-10 flex items-center justify-center gap-14">
                <button type="button" onClick={handleCloseDeleteFood} className="text-[0.95rem] font-medium text-red-500 transition-colors hover:text-red-600">Cancelar</button>
                <button type="submit" disabled={isDeletingFood} className="min-w-37.5 rounded-full bg-red-600 px-8 py-3 text-[0.95rem] font-semibold text-white transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-70">{isDeletingFood ? 'Deletando...' : 'Deletar'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isCreateFoodOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4" onClick={() => setIsCreateFoodOpen(false)}>
          <div className="w-full max-w-[96vw] rounded-3xl bg-white p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-4">
              <div>
                <h2 className="text-2xl font-semibold text-slate-900">Cadastrar alimento</h2>
              </div>

              <button type="button" onClick={() => setIsCreateFoodOpen(false)} className="rounded-full p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900" aria-label="Fechar popup">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><path d="M18 6 6 18" /><path d="m6 6 12 12" /></svg>
              </button>
            </div>

            <form className="mt-5 grid grid-cols-1 gap-6 lg:grid-cols-[360px_minmax(0,1fr)]" onSubmit={handleCreateFood}>
              <div className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-slate-50 p-5">
                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Imagem do alimento</h3>
                </div>
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div className="lg:col-span-2">
                  ...
                  <input type="text" placeholder="Digite o nome principal" value={formData.nomePrincipal} onChange={(event) => setFormData((current) => ({ ...current, nomePrincipal: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition-colors placeholder:text-slate-400 focus:border-blue-500" />
                </div>
                ...
                <div className="lg:col-span-2">
                  <FoodGroupSelect
                    value={formData.grupoAlimentar}
                    onChange={(value) => setFormData((current) => ({ ...current, grupoAlimentar: value }))}
                  />
                </div>

                <div className="lg:col-span-2">
                  <label className="mb-2 block text-sm font-medium text-slate-700">Texto informativo</label>
                  <textarea placeholder="Digite o texto informativo" value={formData.textoInformativo} onChange={(event) => setFormData((current) => ({ ...current, textoInformativo: event.target.value }))} className="min-h-28 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition-colors placeholder:text-slate-400 focus:border-blue-500" />
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-2 lg:col-span-2">
                  <button type="button" onClick={handleCloseCreateFood} className="rounded-full border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50">Cancelar</button>
                  <button type="submit" disabled={isSavingFood} className="rounded-full bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-blue-700">{isSavingFood ? 'Salvando...' : 'Salvar alimento'}</button>
                </div>
              </div>
            </form>
          </div>
    ...
    <tr className="border-b border-slate-200 bg-white">
      <td className="px-2 py-6">
        {imagePreview ? (
          <div className="h-14 w-14 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
            <img src={imagePreview} alt={`Pré-visualização de ${nomePrincipal}`} className="h-full w-full object-cover" />
          </div>
        ) : (
      ...
      <td className="px-2 py-6">
        <div className="flex items-center justify-center gap-4">
          ...
          <button type="button" onClick={onEdit} aria-label={`Editar alimento ${nomePrincipal}`} className="text-slate-500 transition-colors hover:text-blue-600">
            <EditIcon />
          </button>
          <button type="button" onClick={onDelete} aria-label={`Excluir alimento ${nomePrincipal}`} className="text-slate-500 transition-colors hover:text-red-600">
            <TrashIcon />
          </button>
        </div>
      </td>
